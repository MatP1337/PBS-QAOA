...
def inv_prepare_workaround(G, root, N, q_array):

    def inv_prepare(root, N, q_array):
        with invert():
...
def mixer(G, root, N):
    ...
    def apply_mixer(q_array, beta):
        ...
        inv_prepare = inv_prepare_workaround(G, root, N, q_array)

        with conjugate(inv_prepare)(root, N, q_array):
...
def mixer2(G, root, N):
    ...
    def apply_mixer(q_array, beta):
        ...
        qubit_list = sum([list(qv) for qv in q_array], [])
        with conjugate(inv_prepare)(root, N, q_array):
            mcp(beta, qubit_list, ctrl_state = 0)
        return q_array
    return apply_mixer


def custom_mixer(G, root, N):
    ...
    def apply_mixer(q_array, beta):
        ...
        with conjugate(inv_prepare)(root, N, q_array):
            for i in range(len(q_array)):
                mcp(beta[i], q_array[i], ctrl_state = 0)
        return q_array
